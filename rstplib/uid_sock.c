...
int UiD_SocketCompare(UID_SOCKET_T *s, UID_SOCKET_T *t)
{
	register char *ps;
	register char *pt;
...
int UiD_SocketInit(UID_SOCKET_T *s, UID_SOCK_ID socket_id,
		   TYPE_OF_BINDING binding)
{
	...
	switch (binding) {
		case UID_BIND_AS_CLIENT:
			{
				char tmp_name[13] = "rstpd_XXXXXX";
				if (mkstemp(tmp_name) == -1) {
					return -2;
				}
				strncpy(s->socket_id, tmp_name, SOCKET_NAME_LENGTH);
				UID_SET_SOCKET_CLIENT_ID(s,s->socket_id);
				if (bind(s->sock_fd, (SA *)&(s->clientAddr),
					SIZE_OF_ADDRESS) < 0) {
					return -2;
				}
		...
		case UID_BIND_AS_SERVER:
			...
			if (bind(s->sock_fd, (SA *)&(s->serverAddr),
				SIZE_OF_ADDRESS) < 0) {
				perror("Error:");
				fflush(stdout);
				return -4;
...
int UiD_SocketRecvfrom(UID_SOCKET_T *sock, void *msg, int buffer_size,
		       UID_SOCKET_T *sock_4_reply)
{
	...
	while (1) {
		size = recvfrom(sock->sock_fd, msg, buffer_size, 0,
		                (struct sockaddr *)(((UID_BIND_AS_CLIENT
		                                == sock->binding)
		                                || !sock_4_reply) ? NULL
		                                : &(sock_4_reply->clientAddr)),
		                (UID_BIND_AS_CLIENT == (sock->binding)) ? NULL
		                                : &len);
...
int UiD_SocketSendto(UID_SOCKET_T *sock, void *msg, int msg_size)
{
	...
	while (size != msg_size) {
		rc = sendto(sock->sock_fd,
			    (msg+size),
			    (msg_size-size),
			    0,
			    (struct sockaddr *)((UID_BIND_AS_CLIENT
				== sock->binding) ? &sock->serverAddr
					: &sock->clientAddr),
						SIZE_OF_ADDRESS);
...
{
	close(s->sock_fd);
	if (s->binding == UID_BIND_AS_CLIENT) {
		if (remove(s->socket_id) != 0) {
			printf("Warning: could not remove socket file!\n");
		}
	}
	return 0;
}
