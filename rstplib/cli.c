...
static int cli_parse_parms(const char *line, int skip_words, char **argv) {
...
static int help_on_param(int iii, CMD_PAR_DSCR_T *par, int cry_on_empty)
{
	...
	switch (par->param_type) {
		case CMD_PAR_NUMBER:
			printf("; integer in [%ld, %ld]",
			       par->number_limits.min,
...
static void cli_help_brosed_line(int argc, char **argv, const char *line)
{
...
static int cli_call_callback(CMD_DSCR_T *reg, const char *line,
			     int *argc, char **argv)
{
	int cnt;
...
char **cli_private_completion(char *text, int start, int end)
{
	char **matches = NULL;
...
void rl_init()
{
	...
	rl_callback_handler_install(get_prompt(), rl_read_cli);
  	...
  	rl_attempted_completion_function
	                = (CPPFunction *)cli_private_completion;
