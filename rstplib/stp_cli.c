...
static void show_rstp_port(BITMAP_T *ports_bitmap, int detail)
{
	UID_STP_STATE_T uid_state;
	...
	for (port_index = 0; port_index <= NUMBER_OF_PORTS; port_index++) {
		...
		if (detail) {
			...
			if (UID_PORT_DISABLED == uid_port.state || '-'
			                == uid_port.role) {
#if 0
				printf("helloWhen:       %3d  ",
...
static void set_bridge_cfg_value(unsigned long value, unsigned long val_mask)
{
	UID_STP_CFG_T uid_cfg;
	char *val_name;
	int rc;
	...
	if (0 != rc) {
		printf("Can't change rstp bridge %s:%s", val_name,
		       STP_IN_get_error_explanation(rc));
	} else {
...
static void set_rstp_port_cfg_value(int port_index, unsigned long value,
...
static int get_bool_arg(int narg, int argc, char **argv, unsigned long *value)
{
	switch (argv[narg][0]) {
		case 'y':
		case 'Y':
...
static CMD_DSCR_T lang[] = {
	...
	THE_COMMAND("bridge forceVersion", "set bridge forceVersion")
	PARAM_BOOL("forceVersion", "force slow", "regular", "no")
	...
	PARAM_ENUM_SEL("brdgdet", "brdgdet")
	PARAM_ENUM_SEL("transmit", "transmit")
	PARAM_ENUM_SEL("info", "info")
	PARAM_ENUM_SEL("rolesel", "rolesel")
