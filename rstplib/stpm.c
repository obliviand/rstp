...
void _stp_stpm_init_data(STPM_T *this)
{
	STP_VECT_create(&this->rootPriority,
			&this->BridgeIdentifier,
			0,
			&this->BridgeIdentifier,
			0, 0);

	this->BridgeTimes.MessageAge = 0;

	STP_copy_times(&this->rootTimes, &this->BridgeTimes);
}

static unsigned char _check_topoch(STPM_T *this)
{
	register PORT_T *port;
...
void STP_stpm_one_second(STPM_T *param) {
	...
	this->Topology_Change = _check_topoch (this);
	if (this->Topology_Change) {
		this->Topology_Change_Count++;
		this->Time_Since_Topology_Change = 0;
	} else {
		this->Topology_Change_Count = 0;
		this->Time_Since_Topology_Change++;
	}
}
...
void STP_stpm_delete(STPM_T *this)
{
	register STPM_T *tmp;
...
void STP_stpm_stop(STPM_T *this)
{
}
...
BRIDGE_ID *STP_compute_bridge_id(STPM_T *this)
{
	...
	memcpy(this->BridgeIdentifier.addr, old, sizeof(old));
...
void STP_stpm_update_after_bridge_management(STPM_T *this)
{
	...
	/* Re-initialize the whole set of state machines and ports
	 * since there is new information available.
	 */
	_stp_stpm_init_data (this);
...
int STP_stpm_check_bridge_priority(STPM_T *this)
{
	register STPM_T *oth;

	for (oth = bridges; oth; oth = oth->next) {
		if (STP_ENABLED == oth->admin_state && oth != this &&
				! STP_VECT_compare_bridge_id (&this->BridgeIdentifier, &oth->BridgeIdentifier)) {
			return STP_Invalid_Bridge_Priority;
		}
	}
...
const char* STP_stpm_get_port_name_by_id(STPM_T *this, PORT_ID portId)
{
	register PORT_T *port;
